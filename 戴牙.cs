...
namespace Test
{
    public partial class 戴牙 : Form
    {
        ...
        {
            String sqlText = "SELECT 姓名,种植牙位,基台编号,基台类型,基台直径,基台高度,修复体类型,种植体系统,戴牙后备注 FROM [患者代码-姓名视图] INNER JOIN [牙位-患者表] ON [患者代码-姓名视图].患者代码 = [牙位-患者表].患者代码 INNER JOIN 戴牙 ON [牙位-患者表].牙位ID = 戴牙.牙位ID INNER JOIN 基台表 ON 戴牙.基台编号 = 基台表.基品编号";
